...
USAGEMESSAGE="Usage: vb-create-guest.sh -n VMNAME -v VRDEPORT [-o OSTYPE=$OSTYPE -i INITIALISO=$INITIALISO -m MEMORY=$MEMORY -c CPUS=$CPUS -b BRIDGEINTERFACE=$BRIDGEON -h HDSIZE=$HDSIZE -a AUTOSTARTENABLED=$AUTOSTART -d AUTOSTARTDELAY=$AUTOSTARTDELAY]"
