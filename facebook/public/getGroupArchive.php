...
if (isset($_POST["gid"]))
{
    ...
    if ($gid + 0 == $gid && is_int($gid + 0))
    {
        ...
        $dbPhoto     = mysqli_prepare($db, "insert into photos (gid, photo_id, attached_to, attached_type, album_id, owner_id, src, caption, permalink) values (?, ?, ?, ?, ?, ?, ?, ?, ?) on duplicate key update caption = values(caption)");
        $dbPhotoSrc  = mysqli_prepare($db, "update photos set src_big = ? where photo_id = ?");
        $dbUser      = mysqli_prepare($db, "replace into users (user_id, first_name, last_name, name) values (?, ?, ?, ?)");

        if ( ! $dbPost || ! $dbComment || ! $dbPhoto || ! $dbPhotoSrc || ! $dbUser)
        {
        ...
        $bindResult  = mysqli_stmt_bind_param($dbPhoto, "issssisss", $_gid, $_photo_id, $_attached_to, $_attached_type, $_album_id, $_owner_id, $_src, $_caption, $_permalink) && $bindResult;
        $bindResult  = mysqli_stmt_bind_param($dbPhotoSrc, "ss", $_src_big, $_photo_id) && $bindResult;
